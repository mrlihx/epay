...
<?php
$id = intval($_GET['id']);
$row = $DB->find('complain', 'channel,source', ['id'=>$id]);
...
<div class="panel-heading"><h3 class="panel-title">协商历史记录<?php if(!($model::$paytype=='alipay' && $row['status']==2)){?><button onclick="replyForm()" class="pull-right btn btn-sm btn-warning" target="_blank">回复用户</button></h3><?php }?></div>
    <ul class="timeline">
